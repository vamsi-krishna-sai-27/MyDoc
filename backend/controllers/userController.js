...
const bookAppointment = async (req, res) => {
  try {
    const userId = req.userId; // ✅ get it from token
    let { docId, slotDate, slotTime } = req.body;

    // 🛠️ Ensure full date format "DD_MM_YYYY"
    // If frontend sends a parseable date (ISO or similar), convert it.
    let dateObj = null;
    if (slotDate) {
    ...
    if (dateObj && !isNaN(dateObj)) {
      ...
      // fallback: if frontend sends only day number (like "29") or something else
      const today = new Date();
      // if slotDate is numeric string (day), use that day with current month/year
      const dd = Number(slotDate);
      if (!isNaN(dd) && dd >= 1 && dd <= 31) {
        slotDate = `${dd}_${today.getMonth() + 1}_${today.getFullYear()}`;
      } else {
        // if nothing meaningful, use today's date
    ...
    const appointmentData = {
      ...
      amount: docData.fees,
      slotTime,
      slotDate, // ✅ Now it’s like "4_11_2025"
      date: Date.now(),
      cancelled: false,
      payment: false,
      isCompleted: false,
...
const listAppointment = async (req, res) => {
  try {
    ...
    // return latest first and use lean for plain objects
...
const cancelAppointment = async (req, res) => {
  try {
    ...
    // releasing doctor slot (if doctor still exists)
